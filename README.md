...
The internals are pretty simple, but packaged into a flexible ~~battlestation~~ interface. Basically, we run `ASCII(SUBSTRING(query, idx, 1))` to obtain the numeric value of each character, then use binary search to deduce each value.
